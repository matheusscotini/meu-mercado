...
- Banco modelado da seguinte forma:
  - `products` (id, name, price, qty_stock)
  - `orders` (id, customer_name, delivery_date, total_amount)
  - `order_items` (id, order_id, product_id, quantity, unit_price, subtotal)
- A lógica de negócio (validação de estoque, débito e cálculo de total) é feita dentro de **transação** no Laravel para garantir consistência.
